...
class UpdatePhotoCubit extends Cubit<UpdatePhotoState> {
  ...
  UpdatePhotoCubit(
      {required this.imagePicker, required this.uploadPhotoUseCase})
      : super(UpdatePhotoState(updatePhotoState: ViewData.initial()));
  ...
  void uploadPhoto() async {
    ...
    if (image != null) {
      emit(UpdatePhotoState(updatePhotoState: ViewData.loading()));

      final uploadPhoto = await uploadPhotoUseCase(File(image.path));
      uploadPhoto.fold(
        (failure) => emit(UpdatePhotoState(
      ...
      emit(UpdatePhotoState(updatePhotoState: ViewData.loaded()));
    }
  }
}
