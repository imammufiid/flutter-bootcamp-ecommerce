...
class SignUpBloc extends Bloc<SignUpEvent, SignUpState> {
  ...
  SignUpBloc({
  ...
  }) : super(SignUpState(signUpState: ViewData.initial())) {
    on<UsernameChange>((event, emit) {
      if (event.username.isEmpty) {
        ...
        emit(SignUpState(signUpState: ViewData.initial()));
      }
    });
    ...
    on<PasswordConfirmationChange>((event, emit) {
      if (event.passwordConfirmation.isEmpty) {
        emit(
          SignUpState(
            signUpState: ViewData.error(
      ...
      } else if (event.passwordConfirmation != event.password) {
        emit(
          SignUpState(
            signUpState: ViewData.error(
              message: AppConstants.errorKey.confirmPassword,
              failure: FailureResponse(
                  errorMessage:
                      AppConstants.errorMessage.confirmPasswordMustSame),
            ),
          ),
        );
      } else {
    ...
    on<SignUp>((event, emit) async {
      emit(SignUpState(signUpState: ViewData.loading()));
      if (event.authRequestEntity.username.isNotEmpty &&
          event.authRequestEntity.password.isNotEmpty) {
        final result = await signUpUseCase(event.authRequestEntity);
        result.fold(
            (failure) => emit(
                  SignUpState(
                    signUpState: ViewData.error(
        ...
        emit(SignUpState(signUpState: ViewData.loaded()));
      } else {
        emit(
          SignUpState(
            signUpState: ViewData.error(
